...
Route::get('/classrooms/download', [\App\Http\Controllers\ClassroomController::class, 'download']);

Route::middleware('auth:web')->resource('/classrooms', \App\Http\Controllers\ClassroomController::class)
    ->parameters(['{classroom}' =>'{class}'])
    ->name(
        'index' , 'c.index'
    );

Route::get('/login', [\App\Http\Controllers\LoginController::class, 'create'])->name('login')->middleware('guest');
Route::post('/login', [\App\Http\Controllers\LoginController::class, 'store'])->middleware('guest:web');
